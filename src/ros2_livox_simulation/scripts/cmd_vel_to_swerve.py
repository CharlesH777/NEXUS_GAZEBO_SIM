...
class CmdVelToSwerve(Node):
    def __init__(self) -> None:
        ...
        self.use_swerve_kinematics = bool(
            self.declare_parameter("use_swerve_kinematics", True).value
        )
    ...
    def on_timer(self) -> None:
        ...
        if not self.use_swerve_kinematics:
            left_speed = (cmd.linear.x - cmd.angular.z * 0.5 * self.track_width) / self.wheel_radius
            right_speed = (cmd.linear.x + cmd.angular.z * 0.5 * self.track_width) / self.wheel_radius
            peak_speed = max(abs(left_speed), abs(right_speed))
            if peak_speed > self.max_wheel_speed and peak_speed > 1e-6:
                scale = self.max_wheel_speed / peak_speed
                left_speed *= scale
                right_speed *= scale
            ...
            wheel_msg.data = [left_speed, right_speed, left_speed, right_speed]
