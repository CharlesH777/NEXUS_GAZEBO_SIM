...
World::World(const rclcpp::Node::SharedPtr& node, const float &resolution)
    : node_(node),
      ...
      tf_listener_(tf_buffer_)
{
    ...
    PointCloud_Map_topic = node_->declare_parameter<std::string>("PointCloud_Map_topic", PointCloud_Map_topic);
    Grid_Map_topic = node_->declare_parameter<std::string>("Grid_Map_topic", Grid_Map_topic);
    PointCloud_topic = node_->declare_parameter<std::string>("PointCloud_topic", PointCloud_topic);
    
    use_ex_range_ = node_->declare_parameter<bool>("use_ex_range", use_ex_range_);
    ex_robot_back_ = node_->declare_parameter<double>("ex_robot_back", ex_robot_back_);
    ex_robot_front_ = node_->declare_parameter<double>("ex_robot_front", ex_robot_front_);
    ex_robot_right_ = node_->declare_parameter<double>("ex_robot_right", ex_robot_right_);
    ex_robot_left_ = node_->declare_parameter<double>("ex_robot_left", ex_robot_left_);
    
    grid_map_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(Grid_Map_topic, rclcpp::QoS(1));
}
...
{   
    ...
    {
        RCLCPP_ERROR(node_->get_logger(), "Can not initialize the map with an empty point cloud!");
        return;
    }
    clearMap();
    cloud_near_.clear();
...
void World::setObs(const Vector3d &point)
{
    ...
    // Bounds check before array access to prevent crash from LiDAR outliers
    if (!isInsideBorder(idx)) {
        return;
    }
