...
namespace FitPlane
{
    ...
    void PlaneMap::PointCloudMapCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& point_cloud_map)
    {
        // Slam-sim-out or Fast-Lio
        pcl::PointCloud<pcl::PointXYZ> cloud;
        pcl::fromROSMsg(*point_cloud_map, cloud);
    ...
    {
        max_angle_ =
            world_->node_->declare_parameter<double>("max_angle_deg", max_angle_);
        max_flatness_ =
            world_->node_->declare_parameter<double>("max_flatness", max_flatness_);
        w1_ =
            world_->node_->declare_parameter<double>("angle_weight", w1_);
        occThre_ =
            world_->node_->declare_parameter<int>("occupied_value", occThre_);
        flatThre_ =
            world_->node_->declare_parameter<int>("inflation_value", flatThre_);
        nobs_ =
            world_->node_->declare_parameter<int>("near_obstacle_value", nobs_);

        world_->Plane_Map_topic =
            world_->node_->declare_parameter<std::string>("Plane_Map_topic", world_->Plane_Map_topic);
        world_->plane_map_pub_ =
            world_->node_->create_publisher<sensor_msgs::msg::PointCloud2>(world_->Plane_Map_topic, rclcpp::QoS(1));
        world_->point_cloud_map_sub_ =
            world_->node_->create_subscription<sensor_msgs::msg::PointCloud2>(
                world_->PointCloud_Map_topic,
                rclcpp::SensorDataQoS(),
                std::bind(&FitPlane::PlaneMap::PointCloudMapCallback, this, std::placeholders::_1));

        plane_OccMap_pub_ =
            world_->node_->create_publisher<nav_msgs::msg::OccupancyGrid>("plane_OccMap", rclcpp::QoS(1));
        trav_cloud_pub_ =
            world_->node_->create_publisher<sensor_msgs::msg::PointCloud2>(
                "local_traversibility_ponit_cloud", rclcpp::SensorDataQoS());
        pubExploredArea =
            world_->node_->create_publisher<sensor_msgs::msg::PointCloud2>("/explored_areas", rclcpp::QoS(5));
        exploredAreaDwzFilter.setLeafSize(exploredAreaVoxelSize, exploredAreaVoxelSize, exploredAreaVoxelSize);

        RCLCPP_INFO(
            world_->node_->get_logger(),
            "fitplane thresholds: max_angle_deg=%.2f max_flatness=%.4f angle_weight=%.2f occupied_value=%d inflation_value=%d near_obstacle_value=%d",
            max_angle_,
            max_flatness_,
            w1_,
            occThre_,
    ...
    bool PlaneMap::InitPlaneMap()
    {
        ...
        {
            RCLCPP_ERROR(world_->node_->get_logger(), "No world!");
    ...
    bool PlaneMap::getPlaneMap()
    {
        ...
        // Fix off-by-one: valid indices are 0..index_num_-1, so upper bound should be min(index_num_, ...)
        // Previous code allowed i==index_num_(0) which is out of bounds
        int upperx = std::min(index_num_(0), r_x + int(20.0 / resolution_) + 1);
        int uppery = std::min(index_num_(1), r_y + int(20.0 / resolution_) + 1);
    ...
    void PlaneMap::visSurf(
        const PlaneMap &planemap,
        const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr& surf_vis_pub)
    {
        if (!surf_vis_pub)
            return;
        ...
        map_vis.header.stamp = world_->node_->get_clock()->now();
    ...
    bool PlaneMap::pubPlaneGridMap(const PlaneMap &planemap)
    {
        ...
        plane_Occmap_.header.stamp = world_->node_->get_clock()->now();
        plane_OccMap_pub_->publish(plane_Occmap_);

        int r_x = CONTXY2DISC(world_->ego_position_.x - leftdownbound_(0), resolution_); 
        int r_y = CONTXY2DISC(world_->ego_position_.y - leftdownbound_(1), resolution_);
        pcl::PointCloud<pcl::PointXYZI>::Ptr trav_point_cloud = pcl::make_shared<pcl::PointCloud<pcl::PointXYZI>>();
        ...
        trav_point_cloud_msg.header.stamp = world_->node_->get_clock()->now();
        trav_point_cloud_msg.header.frame_id = "map";
        trav_cloud_pub_->publish(trav_point_cloud_msg);// publish downsample point cloud
