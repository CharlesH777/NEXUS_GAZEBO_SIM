...
void odometryHandler(const nav_msgs::msg::Odometry::ConstSharedPtr odom)
{
  ...
  const auto& geoQuat = odom->pose.pose.orientation;
...
int main(int argc, char** argv)
{
  ...
  pathFolder = node->declare_parameter<string>("pathFolder", pathFolder);
  vehicleLength = node->declare_parameter<double>("vehicleLength", vehicleLength);
  vehicleWidth = node->declare_parameter<double>("vehicleWidth", vehicleWidth);
  ...
  laserVoxelSize = node->declare_parameter<double>("laserVoxelSize", laserVoxelSize);
  terrainVoxelSize = node->declare_parameter<double>("terrainVoxelSize", terrainVoxelSize);
  useTerrainAnalysis = node->declare_parameter<bool>("useTerrainAnalysis", useTerrainAnalysis);
  checkObstacle = node->declare_parameter<bool>("checkObstacle", checkObstacle);
  checkRotObstacle = node->declare_parameter<bool>("checkRotObstacle", checkRotObstacle);
  adjacentRange = node->declare_parameter<double>("adjacentRange", adjacentRange);
  obstacleHeightThre = node->declare_parameter<double>("obstacleHeightThre", obstacleHeightThre);
  groundHeightThre = node->declare_parameter<double>("groundHeightThre", groundHeightThre);
  costHeightThre = node->declare_parameter<double>("costHeightThre", costHeightThre);
  costScore = node->declare_parameter<double>("costScore", costScore);
  useCost = node->declare_parameter<bool>("useCost", useCost);
  pointPerPathThre = node->declare_parameter<int>("pointPerPathThre", pointPerPathThre);
  minRelZ = node->declare_parameter<double>("minRelZ", minRelZ);
  maxRelZ = node->declare_parameter<double>("maxRelZ", maxRelZ);
  maxSpeed = node->declare_parameter<double>("maxSpeed", maxSpeed);
  dirWeight = node->declare_parameter<double>("dirWeight", dirWeight);
  dirThre = node->declare_parameter<double>("dirThre", dirThre);
  dirToVehicle = node->declare_parameter<bool>("dirToVehicle", dirToVehicle);
  pathScale = node->declare_parameter<double>("pathScale", pathScale);
  minPathScale = node->declare_parameter<double>("minPathScale", minPathScale);
  pathScaleStep = node->declare_parameter<double>("pathScaleStep", pathScaleStep);
  pathScaleBySpeed = node->declare_parameter<bool>("pathScaleBySpeed", pathScaleBySpeed);
  minPathRange = node->declare_parameter<double>("minPathRange", minPathRange);
  pathRangeStep = node->declare_parameter<double>("pathRangeStep", pathRangeStep);
  pathRangeBySpeed = node->declare_parameter<bool>("pathRangeBySpeed", pathRangeBySpeed);
  pathCropByGoal = node->declare_parameter<bool>("pathCropByGoal", pathCropByGoal);
  ...
  joyToCheckObstacleDelay =
      node->declare_parameter<double>("joyToCheckObstacleDelay", joyToCheckObstacleDelay);
  goalClearRange = node->declare_parameter<double>("goalClearRange", goalClearRange);
  goalX = node->declare_parameter<double>("goalX", goalX);
  goalY = node->declare_parameter<double>("goalY", goalY);
  goalStaleTime = node->declare_parameter<double>("goalStaleTime", goalStaleTime);

  auto subOdometry = node->create_subscription<nav_msgs::msg::Odometry>(
      "/state_estimation", rclcpp::QoS(5), odometryHandler);

  auto subTerrainCloud = node->create_subscription<sensor_msgs::msg::PointCloud2>(
      "/terrain_map", rclcpp::SensorDataQoS(), terrainCloudHandler);

  auto subGoal = node->create_subscription<geometry_msgs::msg::PointStamped>(
      "/way_point", rclcpp::QoS(5), goalHandler);

  auto subBoundary = node->create_subscription<geometry_msgs::msg::PolygonStamped>(
      "/navigation_boundary", rclcpp::QoS(5), boundaryHandler);

  auto pubPath = node->create_publisher<nav_msgs::msg::Path>("/path", rclcpp::QoS(5));
  nav_msgs::msg::Path path;

  #if PLOTPATHSET == 1
  auto pubFreePaths =
      node->create_publisher<sensor_msgs::msg::PointCloud2>("/free_paths", rclcpp::QoS(2));
  #endif

  // Optional debug publisher for the cropped planner cloud.
  ...
  auto publishStopPath = [&]() {
    path.poses.resize(1);
    path.poses[0].pose.position.x = 0;
    path.poses[0].pose.position.y = 0;
    path.poses[0].pose.position.z = 0;
    path.header.stamp = ToBuiltinTime(odomTime);
    ...
    freePaths2.header.stamp = ToBuiltinTime(odomTime);
  ...
  while (rclcpp::ok()) {
    ...
    if (newLaserCloud || newTerrainCloud) {
      ...
      const double currentTime = g_clock ? g_clock->now().seconds() : 0.0;
      const bool goalStale =
          autonomyMode && (!haveGoal || goalTime < 0.0 || currentTime - goalTime > goalStaleTime);
      if (goalStale) {
        publishStopPath();
        continue;
      }
        ...
        if (selectedGroupID >= 0) {
          ...
          for (int i = 0; i < selectedPathLength; i++) {
            ...
            if (dis <= pathRange / pathScale && dis <= relativeGoalDis / pathScale) {
              ...
              // Fix empty path bug: when first point (i=0) fails distance check, resize(0) produces
              // unusable empty path. Instead, keep at least 1 point as intentional fallback.
              if (i == 0) {
                // First point itself exceeds range - keep it as single-point fallback
                path.poses[0].pose.position.x = pathScale * (cos(rotAng) * x - sin(rotAng) * y);
                path.poses[0].pose.position.y = pathScale * (sin(rotAng) * x + cos(rotAng) * y);
                path.poses[0].pose.position.z = pathScale * z;
                path.poses.resize(1);
              } else {
                path.poses.resize(i);
              }
      ...
      plannerCloud2.header.stamp = ToBuiltinTime(odomTime);
