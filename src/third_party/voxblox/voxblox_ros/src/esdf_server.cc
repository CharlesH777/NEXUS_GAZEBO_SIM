...
void EsdfServer::setupRos() {
  // Set up publisher.
  esdf_pointcloud_pub_ = nh_private_.advertise<sensor_msgs::PointCloud2>(
      "esdf_pointcloud", 1, true);
  esdf_slice_pub_ = nh_private_.advertise<sensor_msgs::PointCloud2>(
      "esdf_slice", 1, true);
  traversable_pub_ = nh_private_.advertise<sensor_msgs::PointCloud2>(
      "traversable", 1, true);

  esdf_map_pub_ = nh_private_.advertise<voxblox_msgs::msg::Layer>(
      "esdf_map_out", 1, false);

  // Set up subscriber.
  esdf_map_sub_ = nh_private_.subscribe<voxblox_msgs::msg::Layer>(
      "esdf_map_in", 1, &EsdfServer::esdfMapCallback, this);
...
void EsdfServer::publishAllUpdatedEsdfVoxels() {
  ...
  esdf_pointcloud_pub_.publish(toPointCloud2Message(pointcloud, world_frame_));
}
...
void EsdfServer::publishSlices() {
  ...
  esdf_slice_pub_.publish(toPointCloud2Message(pointcloud, world_frame_));
}
...
void EsdfServer::publishTraversable() {
  ...
  traversable_pub_.publish(toPointCloud2Message(pointcloud, world_frame_));
}
