...
sensor_msgs::PointCloud2 toPointCloud2Message(
    const pcl::PointCloud<PointT>& pointcloud, const std::string& frame_id) {
  sensor_msgs::PointCloud2 message;
  pcl::toROSMsg(pointcloud, message);
  message.header.frame_id = frame_id;
  message.header.stamp = ros::Time::now().to_msg();
  return message;
}
...
TsdfServer::TsdfServer(const ros::NodeHandle& nh,
  ...
  // Advertise topics.
  surface_pointcloud_pub_ = nh_private_.advertise<sensor_msgs::PointCloud2>(
      "surface_pointcloud", 1, true);
  tsdf_pointcloud_pub_ = nh_private_.advertise<sensor_msgs::PointCloud2>(
      "tsdf_pointcloud", 1, true);
  occupancy_marker_pub_ =
      nh_private_.advertise<visualization_msgs::MarkerArray>("occupied_nodes",
                                                             1, true);
  tsdf_slice_pub_ = nh_private_.advertise<sensor_msgs::PointCloud2>(
      "tsdf_slice", 1, true);

  nh_private_.param("pointcloud_queue_size", pointcloud_queue_size_,
                    pointcloud_queue_size_);
  pointcloud_sub_ =
      nh_.subscribe<sensor_msgs::PointCloud2>("pointcloud",
                                              pointcloud_queue_size_,
                                              &TsdfServer::insertPointcloud,
                                              this);

  mesh_pub_ =
      nh_private_.advertise<voxblox_msgs::msg::Mesh>("mesh", 1, true);

  // Publishing/subscribing to a layer from another node (when using this as
  // a library, for example within a planner).
  tsdf_map_pub_ = nh_private_.advertise<voxblox_msgs::msg::Layer>(
      "tsdf_map_out", 1, false);
  tsdf_map_sub_ = nh_private_.subscribe<voxblox_msgs::msg::Layer>(
  ...
  publish_pointclouds_srv_ =
      nh_private_.advertiseService<std_srvs::srv::Empty>(
          "publish_pointclouds", &TsdfServer::publishPointcloudsCallback,
          this);
...
void TsdfServer::processPointCloudMessageAndInsert(
    const sensor_msgs::PointCloud2::SharedPtr& pointcloud_msg,
  ...
  if (enable_icp_) {
    ...
    tf_broadcaster_.sendTransform(
        tf::StampedTransform(icp_tf_msg, ros::Time(pointcloud_msg->header.stamp),
                             world_frame_, icp_corrected_frame_));
    tf_broadcaster_.sendTransform(
        tf::StampedTransform(pose_tf_msg,
                             ros::Time(pointcloud_msg->header.stamp),
...
bool TsdfServer::getNextPointcloudFromQueue(
    std::queue<sensor_msgs::PointCloud2::SharedPtr>* queue,
    sensor_msgs::PointCloud2::SharedPtr* pointcloud_msg,
...
void TsdfServer::insertPointcloud(
    const sensor_msgs::PointCloud2::SharedPtr& pointcloud_msg_in) {
  const ros::Time message_stamp(pointcloud_msg_in->header.stamp);
  if (message_stamp - last_msg_time_ptcloud_ >
      min_time_between_msgs_) {
    last_msg_time_ptcloud_ = message_stamp;
...
void TsdfServer::insertFreespacePointcloud(
    const sensor_msgs::PointCloud2::SharedPtr& pointcloud_msg_in) {
  const ros::Time message_stamp(pointcloud_msg_in->header.stamp);
  if (message_stamp - last_msg_time_freespace_ptcloud_ >
      min_time_between_msgs_) {
    last_msg_time_freespace_ptcloud_ = message_stamp;
    // So we have to process the queue anyway... Push this back.
...
void TsdfServer::publishAllUpdatedTsdfVoxels() {
  ...
  tsdf_pointcloud_pub_.publish(toPointCloud2Message(pointcloud, world_frame_));
}
...
void TsdfServer::publishTsdfSurfacePoints() {
  ...
  surface_pointcloud_pub_.publish(
      toPointCloud2Message(pointcloud, world_frame_));
}
...
void TsdfServer::publishSlices() {
  ...
  tsdf_slice_pub_.publish(toPointCloud2Message(pointcloud, world_frame_));
}
