...
void GraphManager::saveGraph(const std::string& path) {
  ...
  rclcpp::Serialization<planner_msgs::Graph> serializer;
  rclcpp::SerializedMessage serialized;
  serializer.serialize_message(&graph_msg, &serialized);
  auto& raw = serialized.get_rcl_serialized_message();

  // Write to a file
  std::ofstream wrt_file(path, std::ios::out | std::ios::binary);
  wrt_file.write(reinterpret_cast<const char*>(raw.buffer), raw.buffer_length);
  wrt_file.close();
...
void GraphManager::loadGraph(const std::string& path) {
  ...
  std::vector<uint8_t> buffer((std::istreambuf_iterator<char>(read_file)),
                              std::istreambuf_iterator<char>());
  read_file.close();

  rclcpp::SerializedMessage serialized(buffer.size());
  auto& raw = serialized.get_rcl_serialized_message();
  std::memcpy(raw.buffer, buffer.data(), buffer.size());
  raw.buffer_length = buffer.size();

  planner_msgs::Graph graph_msg;
  rclcpp::Serialization<planner_msgs::Graph> serializer;
  serializer.deserialize_message(&serialized, &graph_msg);
