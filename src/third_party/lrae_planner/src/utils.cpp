...
bool Bresenham(
    ...
    std::vector<Eigen::Vector2i> &visited_grid,
    nav_msgs::msg::OccupancyGrid& map)
{
    ...
    if (is_great_than_45_degree){
		while (iy != y2){
            ...
            c_grid[1] = iy;
            visited_grid.push_back(c_grid);
            // Bounds check before array access to prevent crash
            if (ix < 0 || iy < 0 || ix >= static_cast<int>(map.info.width) || iy >= static_cast<int>(map.info.height)) {
                return false;
            }
