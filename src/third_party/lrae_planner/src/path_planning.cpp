...
namespace lrae_planner_ns
{
    ...
    void PathPlanning::setClock(const rclcpp::Clock::SharedPtr& clock)
    {
        clock_ = clock;
    }

    void PathPlanning::setRobotPosition(geometry_msgs::msg::Point robot_position)
    {
    ...
    void PathPlanning::setPlanMap(const nav_msgs::msg::OccupancyGrid& map)
    {
        recmap_ = map;
        clearMap();
        ...
        for (int i = 0; i < width_; i++)
        {
            ...
            {
                // Fix coordinate system: OccupancyGrid is row-major, data[y*width+x]
                // planmap_[i][j] means x=i, y=j, so access should be data[j*width+i] not data[i+j*width]
                if(map.data[j * width_ + i] == -1)
                    planmap_[i][j] = 0;
                else
                    planmap_[i][j] = map.data[j * width_ + i];
            }
        }
    }
    ...
    void PathPlanning::setStart(int startx, int starty)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("path_planning"), "Start Out of map");
            has_start_ = false;
        }
    ...
    void PathPlanning::setGoal(int goalx, int goaly)
    {
        ...
        {
            RCLCPP_INFO(rclcpp::get_logger("path_planning"), "Goal Out of map");
            has_goal_ = false;
        }
    ...
    bool PathPlanning::makePlan(nav_msgs::msg::Path& guiPath, int& pathCost)
    {
    ...
    nav_msgs::msg::Path PathPlanning::getPositionPath(const std::vector<utils_ns::PointInt>& path)
    {
    ...
    bool PathPlanning::finePath(nav_msgs::msg::Path& OriRVPath)
    {
    ...
    bool PathPlanning::getExplorationPath(nav_msgs::msg::Path& exporation_path)
    {   
            ...
            else
            {
                ...
                RCLCPP_ERROR(rclcpp::get_logger("path_planning"), "The goal is out of map");
            }
            int rvcost;
            nav_msgs::msg::Path rvpath;
            ...
            while(!findPath && count_plan < 16)
            {
                for(int i = 0; i < 16; i++)
                {
                    ...
                    // Bounds check BEFORE array access to prevent segfault
                    if (height_ > zy && zy >= 0 && width_ > zx && zx >= 0 &&
                        recmap_.data[zx + zy * recmap_.info.width] < occThre_)
                    {
                        setStart(robot_x_, robot_y_);
                        setGoal(zx, zy);
                        makePlan(rvpath, rvcost);
