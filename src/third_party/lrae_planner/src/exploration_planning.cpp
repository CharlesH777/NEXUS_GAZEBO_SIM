...
{
ExplorationPlanning::ExplorationPlanning(const rclcpp::Node::SharedPtr& node)
    : node_(node),
...
bool ExplorationPlanning::initialize()
{
    ...
    angle_pen_ = node_->declare_parameter<double>("angle_pen", angle_pen_);
    update_cen_thre_ = node_->declare_parameter<int>("update_cen_thre", update_cen_thre_);
    unknown_num_thre_ = node_->declare_parameter<int>("unknown_num_thre", unknown_num_thre_);
    minrange_ = node_->declare_parameter<double>("minrange", minrange_);
    viewpoint_min_path_distance_ =
        node_->declare_parameter<double>("viewpoint_min_path_distance", viewpoint_min_path_distance_);
    viewpoint_max_path_distance_ =
        node_->declare_parameter<double>("viewpoint_max_path_distance", viewpoint_max_path_distance_);
    exploration_path_hold_time_ =
        node_->declare_parameter<double>("exploration_path_hold_time", exploration_path_hold_time_);
    exploration_path_min_end_distance_ =
        node_->declare_parameter<double>("exploration_path_min_end_distance", exploration_path_min_end_distance_);
    limit_max_square_ = node_->declare_parameter<bool>("limit_max_square", limit_max_square_);
    use_go_end_nearest_ = node_->declare_parameter<bool>("use_go_end_nearest", use_go_end_nearest_);
    end_neacen_disthre_ = node_->declare_parameter<double>("end_neacen_disthre", end_neacen_disthre_);
    end_cur_disrate_ = node_->declare_parameter<double>("end_cur_disrate", end_cur_disrate_);

    update_cen_count_ = update_cen_thre_ - 1;

    execution_timer_ = node_->create_wall_timer(
        std::chrono::milliseconds(500),
        std::bind(&ExplorationPlanning::execute, this));
    traversibility_map_sub_ = node_->create_subscription<nav_msgs::msg::OccupancyGrid>(
        traversibility_map_topic_, rclcpp::QoS(100),
    ...
    global_map_sub_ = node_->create_subscription<nav_msgs::msg::OccupancyGrid>(
        global_map_topic_, rclcpp::QoS(100),
        std::bind(&ExplorationPlanning::globalMapCallback, this, std::placeholders::_1));

    exploration_path_pub_ = node_->create_publisher<nav_msgs::msg::Path>("exporation_path", rclcpp::QoS(100));
    exploration_route_pub_ = node_->create_publisher<nav_msgs::msg::Path>("RoutePath", rclcpp::QoS(100));

    robot_position_marker_ =
        std::make_shared<visual::Marker>(node_, "pointvisiual", "map");
    ...
    centroids_marker_ =
        std::make_shared<visual::Marker>(node_, "centroids", "map");
    ...
    windows_marker_ =
        std::make_shared<visual::Marker>(node_, "windows", "map");
    ...
    viewpoints_marker_ =
            std::make_shared<visual::Marker>(node_, "viewpoints", "map");
    ...
    choose_centroids_marker_ =
            std::make_shared<visual::Marker>(node_, "choosecentroids", "map");
    ...
    choose_viewpoints_marker_ =
            std::make_shared<visual::Marker>(node_, "chooseviewpoints", "map");
    ...
    path_planner_.setClock(node_->get_clock());

    return true;
}

void ExplorationPlanning::publishExplorationPath(const nav_msgs::msg::Path& path)
{
    ...
    stamped_path.header.frame_id = "map";
    stamped_path.header.stamp = node_->get_clock()->now();
    exploration_path_pub_->publish(stamped_path);
    last_exploration_path_ = stamped_path;
    last_exploration_path_stamp_ = stamped_path.header.stamp;
    has_last_exploration_path_ = true;
}

bool ExplorationPlanning::reuseHeldExplorationPath(const char* reason)
{
    if (!has_last_exploration_path_ || last_exploration_path_.poses.size() < 2) {
        return false;
    }

    const double held_age =
        (node_->get_clock()->now() - last_exploration_path_stamp_).seconds();
    if (held_age > exploration_path_hold_time_) {
        return false;
    }

    const auto& held_end = last_exploration_path_.poses.back().pose.position;
    if (std::hypot(held_end.x - ego_position_.x, held_end.y - ego_position_.y) <
        exploration_path_min_end_distance_) {
        return false;
    }

    nav_msgs::msg::Path held_path = last_exploration_path_;
    held_path.header.stamp = node_->get_clock()->now();
    exploration_path_pub_->publish(held_path);
    last_exploration_path_stamp_ = held_path.header.stamp;
    RCLCPP_INFO_THROTTLE(
        node_->get_logger(),
        *node_->get_clock(),
        2000,
        "Reusing held exploration path while replanning (%s).",
        reason);
    return true;
}
...
std::vector<int> ExplorationPlanning::getRouteOrder()
{
    ...
    // RCLCPP_INFO(node_->get_logger(), "the best route is:");
    // for (auto &i:best_route) {
    //     RCLCPP_INFO(node_->get_logger(), "%d", i);
    // }
    return best_route;
    // RCLCPP_INFO(node_->get_logger(), "the best unity is %.10f", two_opt_solve.best_unity_);
}
...
bool ExplorationPlanning::getCentroidHelper(const int ori_x, const int& ori_y, const int& w, const int& h, const int& winoder)
{
    ...
    if(isInBorder(tmp_cen.cenindex.x, tmp_cen.cenindex.y))
    {
        ...
        if(isUnKR)
        {
            ...
            if(!tmp_cen.viewpoint_fir.empty()) 
            {
    ...
    {
        RCLCPP_INFO(node_->get_logger(), "Out of map !");
        return false;
    }
}
...
bool ExplorationPlanning::assessUnknownRegion(Centroid* choosecentroid)
{
    ...
    for(int i = 0; i < choosecentroid->viewpoint_fir.size(); i++)
    {
        ...
        else
        {
            ...
            if(pathdisRV > viewpoint_max_path_distance_ || pathdisRV < viewpoint_min_path_distance_)
                continue;
            else
            {
...
{   
    ...
    if (go_home_count_ > 20) {
        RCLCPP_WARN_THROTTLE(
            ...
            *node_->get_clock(),
            2000,
            "There are almost no explorable unknown regions! Continuing to retry as the map updates.");
        go_home_count_ = 20;
    }
    ...
    {
        // RCLCPP_INFO(node_->get_logger(), "Waiting map or robot... \n");
    }
    else
    {
        ...
        if(update_cen_count_ % update_cen_thre_ == 0 || findCenIs_ == false)
        {
            ...
            if(!findCenIs_)
            {
                RCLCPP_INFO(node_->get_logger(), "The current window has no centroid, expand the exploration window size.");
                flag1 = true;
        ...
        else
        {
            ...
            {
                if (reuseHeldExplorationPath("no_centroid_candidates")) {
                    flag1 = false;
                    go_home_count_ = 0;
                } else {
                    // RCLCPP_INFO(node_->get_logger(), "DEBUG: all_outwin_centroids_ is empty");)
                    RCLCPP_WARN(node_->get_logger(), "There are almost no explorable unknown regions!");
                    go_home_count_++;
                }
            }
            else
            {
                    ...
                    && (abs(ego_position_.x - last_near_cen_.centroidposition.x) > 10 || abs(ego_position_.y - last_near_cen_.centroidposition.y) > 10))
                {
                    ...
                    exporation_path.header.stamp = node_->get_clock()->now();
                ...
                if(exporation_path.poses.size() >= 2)
                {
                    publishExplorationPath(exporation_path);
                    RCLCPP_INFO(node_->get_logger(), "Exploring nearby centroid... \n");  
                    flag1 = false;
                    go_home_count_ = 0;
                }
                else if (reuseHeldExplorationPath("nearby_centroid_replan"))
                {
                    flag1 = false;
                    go_home_count_ = 0;
                }
                ...
                {
                    // RCLCPP_INFO(node_->get_logger(), "DEBUG: There is no path to the nearby centroid !");
                    RCLCPP_WARN(node_->get_logger(), "There are almost no explorable unknown regions.");
                    go_home_count_++;
                }
            }
        ...
        if(centroids_.size() > 0 && findCenIs_)
        {
            ...
            if(orders.size() <= 1)
            {
                RCLCPP_INFO(node_->get_logger(), "DEBUG: No route order ! ");
            }
            else
            {
            ...
            if(findnCen == 1)
            {
                ...
                {
                    RCLCPP_INFO(node_->get_logger(), "The evaluated centroid does not have a ahead viewpoint.");
                }
            ...
            else
            {
                ...
                {
                    RCLCPP_INFO(node_->get_logger(), "DEBUG: No centroid route !");
                }
            }
            ...
            {
                // Check viewpoint_fir is not empty to prevent underflow and crash
                if (!centroid1.viewpoint_fir.empty()) {
                    ...
                    // RCLCPP_INFO(node_->get_logger(), "Goal is vp");
                } else {
                    RCLCPP_WARN(node_->get_logger(), "viewpoint_fir is empty, skipping centroid selection");
                }
            }            
            ...
            else
            {
                ...
                if(path_planner_.getExplorationPath(exporation_path))
                {
                    path_planner_.finePath(exporation_path);
                    publishExplorationPath(exporation_path);
                    // RCLCPP_WARN(node_->get_logger(), "Get c2 path");     
                    go_home_count_ = 0;                 
                }
                else if (reuseHeldExplorationPath("route_replan"))
                {
                    go_home_count_ = 0;
                }
                else
                {
                    ...
                    RCLCPP_INFO(node_->get_logger(), "Replan.");
                    go_home_count_++;
                }
            }
        }
        ...
        if(!flag1)
        {
            RCLCPP_WARN(node_->get_logger(), "Exploring ... ");
            // RCLCPP_INFO(node_->get_logger(), "Route and path planning time is :  %0.2f  ms", run_time_s);
        }
    }
}
}
