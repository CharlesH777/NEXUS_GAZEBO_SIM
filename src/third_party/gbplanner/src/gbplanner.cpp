...
void Gbplanner::initializeAttributes() {
  ...
  planner_goto_wp_service_ =
      nh_.advertiseService<planner_msgs::planner_go_to_waypoint>(
  ...
  planner_enable_untraversable_polygon_subscriber_service_ =
      nh_.advertiseService<std_srvs::srv::SetBool>(
  ...
  planner_set_planning_trigger_mode_service_ =
      nh_.advertiseService<planner_msgs::planner_set_planning_mode>(
...
bool Gbplanner::passingGateCallback(
  ...
  res.bound.mode = planner_msgs::BoundMode::K_EXTENDED_BOUND;
  return true;
}
  ...
  if (status == Rrg::GraphStatus::OK) {
    ...
    switch (status) {
      ...
      case Rrg::GraphStatus::NOT_OK:
        ...
        res.status = planner_msgs::planner_srv::Response::K_REPOSITIONING;
        break;
      default:
...
bool Gbplanner::plannerSetPlanningTriggerModeCallback(
  ...
  if (request.planning_mode == request.K_AUTO)
    in_trig_mode = PlannerTriggerModeType::kAuto;
  else if (request.planning_mode == request.K_MANUAL)
