...
8. [街道语义层 — run_street_infer.sh](#8-街道语义层)
...
├── run_street_infer.sh          # Pointcept 街道语义推理
...
└── utils/
    ...
    ├── street_infer.sh          # street_infer 便捷入口（旧路径兼容）
...
| **建图层** | `run_gp_fastlio.sh`, `run_gp_nav.sh`, `run_street_infer.sh` | GP 建图 / 语义推理 |
...
run_street_infer.sh ──> run_sim.sh street + pointcept_semseg_node.py
```
...
- **不含** sand MPC
- **不含** novelty explorer
- MPPI 使用旧版 `mppi_navigator`（Python 节点），不是 Nav2 的 `nav2_mppi_controller`
...
**链路**：Gazebo 街道场景 → 点云管线 → Pointcept PT-v3 语义分割

特殊点：
- 使用 **conda 环境**（Pointcept 的 `.conda-env`），与 `sanitize_python_env` 的清理相反——这里需要 conda
- 通过 `MAP_SIM_ENABLE_DEFAULT_STACK=0` 防止 `run_sim.sh` 分发到其他堆栈
- 传入 `street` 作为地图选择器 → `autoware.world`
- ROS_HOME 和 ROS_LOG_DIR 重定向到仓库内
...
### 12.4 `street_infer.sh`（旧路径兼容）

```bash
exec "$ROOT_DIR/run_sim_street_infer.sh" "$@"
```

注意：这个路径是 `$ROOT_DIR/run_sim_street_infer.sh`（仓库根），不是 `scripts/run_street_infer.sh`。这是一个旧入口的兼容 shim。
...
| `MAP_SIM_FASTLIO2_BIN` | /home/charles/.../lio_node | 二进制路径 |
...
`MAP_SIM_FASTLIO2_BIN` 和 `build_fastlio.sh` 中的 `FASTLIO_WS_DIR` 默认指向 `/home/charles/桌面/slam_2026_charles/...`。更换机器需通过环境变量覆盖。

### CuPy 工作区路径硬编码

`ELEV_WS_DIR` 默认 `/home/charles/NEXUS/tools/elevation_mapping_cupy_ros2_ws`。如果不存在，`run_mppi.sh` / `run_elevation_mppi.sh` 会尝试从 `$ROOT_DIR/tools/elevation_ros2/build_elevation_mapping_ros2.sh` 构建。

### conda 环境冲突

`sanitize_python_env` 会清除所有 conda 相关变量。但 `run_street_infer.sh` 需要 conda（Pointcept），所以它不调用 `sanitize_python_env`，而是显式 `conda activate`。
...
### `monitor_lrae.sh` 硬编码路径

第 27 行 `cd /home/charles/NEXUS/NEXUS_GAZEBO_SIM` 是硬编码的，换机器需要改。
