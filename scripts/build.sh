...
[ -f "$ROOT_DIR/install/setup.bash" ] || { echo "[ERR] Build did not produce $ROOT_DIR/install/setup.bash" >&2; exit 1; }
...
echo "[NEXT] Run the minimal sim: bash ./scripts/run_sim.sh"
echo "[NEXT] Run the Gazebo GUI + RViz view: bash ./scripts/run_sim_gui.sh"
echo "[NEXT] Run the omni variant explicitly: bash ./scripts/run_sim_omni.sh"
...
echo "[NEXT] To test it anyway: MAP_SIM_ALLOW_UNSTABLE_DEPTH_CAMERA=1 bash ./scripts/run_sim_camera.sh"
